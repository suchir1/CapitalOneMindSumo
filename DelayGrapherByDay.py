...
for i in range(len(dataSet['on_scene_timestamp'])):
    ...
    if dataSet['call_date'][i] not in delay_times:
        delay_times[dataSet['call_date'][i]] = 0
    if dataSet['call_date'][i] not in district_freq:
        district_freq[dataSet['call_date'][i]] = 0
    delay_times[dataSet['call_date'][i]] = delay_times[dataSet['call_date'][i]] + on_scene.timestamp()-received.timestamp()
    district_freq[dataSet['call_date'][i]] = district_freq[dataSet['call_date'][i]] +1
